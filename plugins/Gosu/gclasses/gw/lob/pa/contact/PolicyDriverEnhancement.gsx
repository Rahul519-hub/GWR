...
enhancement PolicyDriverEnhancement : entity.PolicyDriver {
  ...
  // Argo_Diver Information
  function DriverInformationPostOnChange(){

    if(this.DriverInformation){

     print("****addArgo_DiverValues*****")

     addArgo_DiverValues()

   }else{

     print("***removeArgo_DiverValues******")

      removeArgo_DiverValues()
   }
}

 // now i am adding two functions above line which is showing error. in between im giving print statement to check its right or worng
     // Simply copy paste the above value and same for (remove)

  function addArgo_DiverValues(){

    if(this.Argo_DiverValues == null){
      this.Argo_DiverValues = new Argo_DiverValues(this.Branch)
    }
// setting default values
    this.Argo_DiverValues.DriverOverride_T = false
    updateCellPhone()
    }
  function updateCellPhone(){
    var cell = (this.AccountContactRole.AccountContact.Contact as Person).CellPhone.toString()
    if (cell!=this.Argo_DiverValues.DriverCellPhone){
      print("*****Argo_DiverValues Doesn't have same value******")
      this.Argo_DiverValues.DriverCellPhone = cell
      print("****Value of the Phone*****" +cell)
    }
  }

  // Now i am adding the 2nd function aboveline which showing error.

   function removeArgo_DiverValues(){
     print("*********Remove Diver Information*********")
    this.Argo_DiverValues?.remove()
